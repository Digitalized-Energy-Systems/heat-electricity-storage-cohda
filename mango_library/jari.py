...
async def test_case(power_target, heat_target, value_weights, schedules_provider):
    ...
    for part_id, a in enumerate(agents):
        ...
        coalition_model.add(coal_id, CoalitionAssignment(coal_id, list(
            filter(lambda a_t: a_t[0] != str(part_id), map(lambda ad: (ad[1], c.addr, ad[0].aid), zip(agents, range(10))))), 'cohda', str(part_id), 'agent0', addr))
    ...
    await asyncio.wait_for(wait_for_term(agents), timeout=1E2)
    ...
    # print("63: ", type(agents[0].roles[0]._cohda[coal_id]._memory.solution_candidate.schedules), agents[0].roles[0]._cohda[coal_id]._memory.solution_candidate.schedules)
    columns = ["schedule", "conversions", "penalty", "value", "schedule", "conversions", "penalty"]
    data = []
    for agent in agents:
        data.append(agent.roles[0]._cohda[coal_id]._last)
    print(f"{format((time.time() - global_start_time) * 1000, '.3f')}ms")
    data.append(np.sum(np.asarray(data, dtype=object), axis=0))
    # TODO pd.DataFrame(data=data, columns=columns) in jari.py
    # print(pd.DataFrame(data=data, columns=columns))
    solution_candidate_schedules_sum = EnergySchedules({'power': power_target, 'heat': heat_target}, {'power_penalty': 1, 'heat_penalty': 1})
    for solution_candidate_schedule in solution_candidate_schedules:
        solution_candidate_schedules_sum += solution_candidate_schedule
    soll = EnergySchedules(dict_schedules={'power': power_target, 'heat': heat_target}, value_weights={'power_penalty': 0, 'heat_penalty': 0})
    print(f'SOLL:\t{soll}')
...
asyncio.run(test_case(
    ...
    value_weights=[
        {'power_penalty': .3, 'heat_penalty': .9, 'gas_price': 0.07, 'power_kwh_price': 1, 'heat_kwh_price': 1, 'converted_price': .4, 'convert_amount': 10, 'max_gas_amount': 000, 'gas_to_heat_factor': .5, 'gas_to_power_factor': .5, 'power_to_heat_factor': .9, 'power_to_heat_amount': 7},
        {'power_penalty': .3, 'heat_penalty': .9, 'gas_price': 0.07, 'power_kwh_price': 1, 'heat_kwh_price': 1, 'converted_price': .5, 'convert_amount': 0, 'max_gas_amount': 000, 'gas_to_heat_factor': .5, 'gas_to_power_factor': .5, 'power_to_heat_factor': .9, 'power_to_heat_amount': 7},
        {'power_penalty': .3, 'heat_penalty': .9, 'gas_price': 0.07, 'power_kwh_price': 1, 'heat_kwh_price': 1, 'converted_price': .6, 'convert_amount': 0, 'max_gas_amount': 000, 'gas_to_heat_factor': .5, 'gas_to_power_factor': .5, 'power_to_heat_factor': .9, 'power_to_heat_amount': 7},
        {'power_penalty': .3, 'heat_penalty': .9, 'gas_price': 0.07, 'power_kwh_price': 1, 'heat_kwh_price': 1, 'converted_price': .7, 'convert_amount': 0, 'max_gas_amount': 000, 'gas_to_heat_factor': .5, 'gas_to_power_factor': .5, 'power_to_heat_factor': .9, 'power_to_heat_amount': 7},
